...
const DEFAULT_BASE = (
  process.env.NEXT_PUBLIC_API_BASE_URL || "http://vitex.duckdns.org/api/v1"
).replace(/\/$/, "");
...
export default function CreateLessonPlanPage() {
  ...
  const [targetOutcomes, setTargetOutcomes] = useState<string[]>([
    ...
    "Rehab",
    "Strength",
  ]); // Default to Conditioning, Technical, Rehab, and Strength
    ...
    try {
      ...
      if (apiUserRaw) {
        try {
          ...
          name =
            apiUser?.name || apiUser?.fullname || apiUser?.username || null;
        } catch (e) {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const res = await fetch(`${DEFAULT_BASE}/users?per_page=1000`, {
        headers: authHeaders(),
      });
  ...
  const saveUserSelection = () => {
    const chosen = users.filter((u: any) =>
  ...
  const [sectionsByDate, setSectionsByDate] = useState<
  ...
  const findDrillById = (id: string | number) =>
    drills.find((d) => String(d.id) === String(id));
  ...
  const onDropToSection = (e: React.DragEvent, sectionKey: string) => {
    ...
    if (activeDate) {
      setSectionsByDate((cur) => {
        const curForDate = cur[activeDate] || {
          "warm-up": [],
          "main-workout": [],
          "cool-down": [],
        };
        const existing = curForDate[sectionKey] || [];
        if (existing.find((it: any) => String(it.id) === String(drill.id)))
          return cur;
        return {
          ...cur,
          [activeDate]: { ...curForDate, [sectionKey]: [...existing, drill] },
        };
      });
      return;
    }
  ...
  const removeFromSection = (sectionKey: string, id: string | number) => {
    if (activeDate) {
      setSectionsByDate((cur) => {
        ...
        return {
          ...
          [activeDate]: {
            ...curForDate,
            [sectionKey]: (curForDate[sectionKey] || []).filter(
              (it: any) => String(it.id) !== String(id),
            ),
          },
        };
      });
    ...
    setSections((cur) => ({
      ...cur,
      [sectionKey]: (cur[sectionKey] || []).filter(
        (it: any) => String(it.id) !== String(id),
      ),
    }));
  };
  ...
  const buildStepsPayload = () => {
    ...
    const targetDates =
      dates.length > 0 ? dates : [new Date().toISOString().slice(0, 10)];
    ...
    const sectionsPayload = targetDates.map((d) => {
      ...
      return {
        ...
        steps: [
          {
            step: "warm-up",
            exercise_ids: (s["warm-up"] || []).map((it) => it.id),
          },
          {
            step: "main-workout",
            exercise_ids: (s["main-workout"] || []).map((it) => it.id),
          },
          {
            step: "cool-down",
            exercise_ids: (s["cool-down"] || []).map((it) => it.id),
          },
        ],
      };
    });
  ...
  const createPlan = async (isSynced: boolean = false) => {
    try {
      ...
      const body = {
        ...
        target_outcome: (targetOutcomes || []).map((t) =>
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      ...
      try {
        const res = await fetch(`${DEFAULT_BASE}/enums/CategoryEnum`, {
          headers: authHeaders(),
        });
        const json = await res.json();
    ...
    const fetchDrills = async () => {
      ...
      try {
        const res = await fetch(
          `${DEFAULT_BASE}/exercises?page=1&per_page=1000`,
          { headers: authHeaders() },
        );
        const json = await res.json();
      ...
      if (!activeDate || !isoDates.includes(activeDate)) {
        ...
        setSectionsByDate((cur) =>
          cur[first]
            ...
            : {
                ...cur,
                [first]: { "warm-up": [], "main-workout": [], "cool-down": [] },
              },
        );
    ...
    setSectionsByDate((cur) =>
      cur[iso]
        ...
        : {
            ...cur,
            [iso]: {
              "warm-up": Array.from(sections["warm-up"] || []),
              "main-workout": Array.from(sections["main-workout"] || []),
              "cool-down": Array.from(sections["cool-down"] || []),
            },
          },
    );
    setStartTimes((cur) =>
      cur[iso] ? cur : { ...cur, [iso]: getCurrentTimeString() },
    );
  };

  // derive which sections to render: per-active-date if set, otherwise the template `sections`
  const templateSections = {
    "warm-up": sections["warm-up"] || [],
    "main-workout": sections["main-workout"] || [],
    "cool-down": sections["cool-down"] || [],
  };
  const displayedSections = activeDate
    ? sectionsByDate[activeDate] || templateSections
    : templateSections;

  // Calculate duration for a section
  const getSectionDuration = (
    sectionKey: "warm-up" | "main-workout" | "cool-down",
  ): number => {
    const sectionExercises =
      (displayedSections as Record<string, any[]>)[sectionKey] || [];
  ...
  const getTotalDuration = (): number => {
    return (
      getSectionDuration("warm-up") +
      getSectionDuration("main-workout") +
      getSectionDuration("cool-down")
    );
  };
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Define the foundational framework for your athlete's upcoming
              performance phase.
            </p>
          </div>
        </div>
      ...
      <div className="rounded-lg border bg-white p-6">
        ...
        {step === 1 && (
          <div>
            <div className="mb-4">
              ...
              <AppInput
                placeholder="e.g., Pre-Season Endurance Phase 1"
                value={planName}
            ...
            <div className="flex gap-4 mb-4 items-start">
              ...
              <div className="min-w-[50%]">
                ...
                <div className="flex gap-2 flex-wrap">
                  {["Conditioning", "Technical", "Rehab", "Strength"].map(
                    (t) => {
                      const selected =
                        targetOutcomes.includes(t) ||
                        (t === "Conditioning" && targetOutcomes.length === 0); // Default to Conditioning
                      return (
                        <button
                          key={t}
                          type="button"
              ...
              <div className="col-span-7">
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        <span className="text-sm text-slate-400">
                          Created by {creatorName} • {createdAtDisplay}
                        </span>
                      </div>

                      <div className="flex items-center gap-3 mt-4">
                        <span className="px-2 py-1 rounded-full bg-slate-50 text-xs">
                          ⚡ High Intensity
                        </span>
                        <span className="px-2 py-1 rounded-full bg-slate-50 text-xs">
                          Difficult
                    ...
                    <div className="flex items-center">
                      <div className="rounded-lg bg-slate-50 px-4 py-3 text-center">
                        ...
                        <div className="text-lg font-semibold text-[#5954E6]">
                ...
                <div className="space-y-6">
                  ...
                  <section className="rounded-lg border-2 border-[#E8F0FF] bg-white p-4 relative">
                    ...
                    <div className="absolute right-4 top-4 text-xs font-semibold text-[#5954E6]">
                      {getSectionDuration("warm-up")} MIN
                    </div>
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      <div className="inline-flex h-6 w-6 items-center justify-center rounded bg-[#EEF4FF] text-[#2B4BFF]">
                        ...
                        >
                          ...
                          />
                          <path
                            d="M11 10h2v6h-2v-6zm0-4h2v2h-2V6z"
                            fill="#2B4BFF"
                          />
                        </svg>
                      </div>
                    ...
                    >
                      {(displayedSections["warm-up"] || []).length === 0 ? (
                        <div className="flex items-center justify-center min-h-[120px] text-sm text-slate-400 opacity-50">
                          Drag & drop to add exercises
                        </div>
                      ) : (
                        (displayedSections["warm-up"] || []).map((s: any) => {
                          ...
                          if (badgeLower.includes("low"))
                            badgeClasses += " bg-emerald-100 text-emerald-700";
                          else if (
                            badgeLower.includes("high") ||
                            badgeLower.includes("difficult")
                          )
                            badgeClasses += " bg-rose-100 text-rose-700";
                          else if (
                            badgeLower.includes("med") ||
                            badgeLower.includes("moderate")
                          )
                            badgeClasses += " bg-amber-100 text-amber-700";
                          else if (
                            badgeLower.includes("tac") ||
                            badgeLower.includes("easy")
                          )
                          ...
                          return (
                            <div
                              key={`warm-${s.id}`}
                              className="rounded-md border border-slate-100 bg-slate-50 p-3 text-sm flex items-start justify-between"
                            >
                              <div>
                              ...
                              <button
                                onClick={() =>
                                  removeFromSection("warm-up", s.id)
                                }
                  ...
                  <section className="rounded-md border p-4 bg-white">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="text-xs font-semibold text-[#5954E6]">
                        {getSectionDuration("main-workout")} MIN
                      </div>
                    </div>
                    ...
                    >
                      {(displayedSections["main-workout"] || []).length ===
                      0 ? (
                        <div className="flex items-center justify-center min-h-[120px] text-sm text-slate-400 opacity-50">
                          Drag & drop to add exercises
                        </div>
                      ) : (
                        (displayedSections["main-workout"] || []).map(
                          (s: any) => {
                            const badge = s.badge || "";
                            const badgeLower = String(badge).toLowerCase();
                            let badgeClasses =
                            ...
                            return (
                              ...
                              >
                                ...
                                <button
                                  onClick={() =>
                                    removeFromSection("main-workout", s.id)
                                  }
                                  className="text-xs text-slate-500"
                                >
                                  Remove
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <div className="inline-flex h-6 w-6 items-center justify-center rounded bg-[#F6F0FF] text-[#6B52E6]">
                          ...
                          >
                            <path
                              d="M12 2l2 4 4 .5-3 3 .8 4L12 12l-3.8 2.5.8-4-3-3 4-.5L12 2z"
                              fill="#F3E8FF"
                            />
                          </svg>
                        </div>
                      ...
                      <div className="text-xs font-semibold text-[#5954E6]">
                        {getSectionDuration("cool-down")} MIN
                      </div>
                    </div>
                        ...
                        (displayedSections["cool-down"] || []).map((s: any) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <button
                                onClick={() =>
                                  removeFromSection("cool-down", s.id)
                                }
                                className="text-xs text-slate-500"
                              >
                                Remove
              ...
              <div className="col-span-5">
                ...
                <aside className="rounded border p-4 bg-white">
                  <AppInput
                    ...
                    value={asideSearch}
                    onChange={(e: any) => setAsideSearch(e.target.value)}
                  />
                  <div className="flex flex-wrap gap-2 mt-3">
                    <button
                      onClick={() => setAsideFilter("All")}
                      className={`px-3 py-1 text-sm rounded-full ${asideFilter === "All" ? "bg-[#f1f5ff]" : "border"}`}
                    >
                      All
                    </button>
                    {categoriesLoading ? (
                      ...
                      (categories.length > 0 ? categories : []).map(
                        (c: any, idx: number) => {
                          const key =
                            typeof c === "string"
                              ? c
                              : (c.key ?? c.value ?? `cat-${idx}`);
                          const label =
                            typeof c === "string"
                              ? c
                              : (c.label ?? c.name ?? String(key));
                          return (
                            <button
                              key={`aside-cat-${String(key)}`}
                              onClick={() => setAsideFilter(String(key))}
                              className={`px-3 py-1 text-sm rounded-full ${asideFilter === String(key) ? "bg-[#f1f5ff]" : "border"}`}
                            >
                              {label}
                            </button>
                          );
                      ...
                      drills
                        .filter((d) => {
                          ...
                          if (
                            ...
                            !String(d.title || "")
                              .toLowerCase()
                              .includes(asideSearch.toLowerCase())
                          )
                            return false;
                  ...
                  <div className="mt-4 text-center text-sm text-[#5954E6]">
                    + Create Custom Drill
                  </div>
                </aside>
              </div>
          ...
          <div>
            <div className="rounded-lg border bg-white p-6 mb-6">
              <div className="flex items-start justify-between">
                <div>
                  <h2 className="text-xl font-semibold">
                    Assign & Generate Sessions
                  </h2>
                  <p className="text-sm text-slate-500 mt-1">
                    Review your schedule and resolve conflicts.
                  </p>
                </div>
                <div>
            ...
            <div className="space-y-6">
              <div className="rounded border bg-white p-4">
                <div className="flex items-center justify-between">
                  <div className="text-sm font-medium">
                    1. Selected Athletes & Teams
                  </div>
                  <div className="text-sm text-slate-500">
                    <button onClick={openUserModal} className="underline">
                      Edit Selection
                    </button>
                  </div>
                </div>
                <div className="mt-3 flex items-center">
                  {selectedUsers &&
                    selectedUsers.slice(0, 3).map((u: any, index: number) => (
                      <div
                        key={`sel-${u.id}`}
                        className={`h-8 w-8 rounded-full flex items-center justify-center ${pickAvatarClass(u.name)} ${index > 0 ? "-ml-2" : ""} border-2 border-white`}
                      >
                  ...
                  {Array.from({
                    length: Math.max(0, 3 - (selectedUsers || []).length),
                  }).map((_, i) => (
                    <div
                      key={`ph-${i}`}
                      className={`h-8 w-8 rounded-full bg-slate-200 border-2 border-white ${(selectedUsers?.length || 0) + i > 0 ? "-ml-2" : ""}`}
                    />
                  ))}
                  <div
                    className={`h-8 w-8 text-xs flex items-center justify-center rounded-full bg-[#eef2ff] text-[#5b21b6] px-2 py-1 -ml-2`}
                  >
                    +{Math.max(0, (selectedUsers?.length || 0) - 3)}
                  </div>
                  <div className="text-sm">
                    Men's Varsity Football Squad ({selectedUsers?.length || 0}{" "}
                    Total)
                  </div>
                </div>
              </div>
              ...
              <div className="rounded border bg-white p-4">
                ...
                <div className="space-y-3">
                  <div className="rounded bg-slate-50 p-3 flex items-center justify-between">
                    <div>
                      ...
                      <div className="font-medium">
                        Session 1: Power Foundations
                      </div>
                      <div className="text-xs text-slate-400">
                        08:00 AM - 09:30 AM • Weight Room
                      </div>
                    </div>
                  ...
                  <div className="rounded bg-slate-50 p-3">
                    ...
                    <div className="font-medium">
                      Session 3: Cardiovascular Engine
                    </div>
                    <div className="text-xs text-slate-400">
                      07:30 AM - 09:00 AM • Main Track
                    </div>
                  </div>
                </div>
        ...
        <div className="flex items-center justify-between mt-6">
          <div className="flex items-center gap-3">
            <Link
              href="/coach/exercises/lesson-plan/list"
          ...
          <div className="flex items-center gap-3">
            {step > 1 && (
              <AppButton
                variant="ghost"
                className="text-[#5954E6] bg-transparent border border-[#5954E6]"
            ...
            {step < 3 ? (
              ...
              <>
                ...
                <AppButton
                  onClick={() => router.push("./pre-preview")}
                  className="bg-[#5954E6] text-white"
                >
                  Start Training
                </AppButton>
              </>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-slate-700 mt-1">
                Defining a clear Target Outcome allows the platform to
                automatically suggest relevant training modules and analytics
                visualizations once the plan is active.
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="relative z-10 w-[720px] max-w-full bg-white rounded shadow-lg p-4">
              ...
              {usersLoading ? (
                ...
                <ul>
                  {(users || []).map((u: any) => (
                    ...
                    >
                      <div
                        className={`h-8 w-8 rounded-full flex items-center justify-center ${pickAvatarClass(u.name || u.fullname || u.username)}`}
                      >
                        <span className="text-xs font-semibold">
                          {getInitials(u.name || u.fullname || u.username)}
                        </span>
                      </div>
                      <div className="flex-1 text-sm">
                        <div className="font-medium">
                          {u.name || u.fullname || u.username || `User ${u.id}`}
                        </div>
                        <div className="text-xs text-slate-400">
                          {u.email || ""}
                        </div>
            ...
            <div className="mt-4 flex items-center justify-end gap-2">
              ...
              <button
                ...
                className="px-3 py-2 bg-[#5954E6] text-white rounded text-sm"
              >
                Save Selection
              </button>
            </div>
